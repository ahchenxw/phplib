...
class Util
{
    ...
    {
        $list = [
            0 => 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789', //大小写字母、数字
            1 => 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',//大小写字母
            2 => '0123456789',//数字
            3 => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ',//大写字母
            4 => 'abcdefghijklmnopqrstuvwxyz',//小写字母
            5 => 'ABCDEFGHIJKMNPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz23456789',// 默认去掉了容易混淆的字符oOLl和数字01
            6 => 'abcdef0123456789',//16进制字符串
        ...
        $type = intval($type);
        $chars = isset($list[$type]) ? $list[$type] : $list[0] . $addChars;

        if ($type === 7) {
            $strLen = mb_strlen($chars, 'utf-8');
            $str = '';
            for ($i = 0; $i < $len; $i++) {
                $str .= mb_substr($chars, mt_rand(0, $strLen - 1), 1, 'utf-8');
            }
        } else {
            //重复字符串次数，执行5选1
            $rep = ceil($len * 5 / strlen($chars));
            if ($rep > 1) $chars = str_repeat($chars, $rep);
